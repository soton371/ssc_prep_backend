...
def create_leaderboard_entry(db: Session, leaderboard_data: LeaderboardCreate) -> Leaderboard:
    ...
    existing_entries = db.query(Leaderboard).filter(Leaderboard.month == current_month).order_by(Leaderboard.rank).all()
    if existing_entries:
...
def get_leaderboard_for_current_month(db: Session, skip: int = 0, limit: int = 10):
    current_month = datetime.now().strftime("%B")
    return db.query(Leaderboard).filter(Leaderboard.month == current_month).order_by(Leaderboard.rank).offset(skip).limit(limit).all()